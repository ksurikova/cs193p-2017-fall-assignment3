    ...
    private func updateCardsState(){
        ...
        if let isMatched = game.isSet {
            for index in 0..<setCardsView.cardViews.count {
                ...
                if game.cardsChosen.contains(card) {setCardsView.cardViews[index].setBorderColor(at: isMatched ? SetGameUIConstants.matchedCardColor : SetGameUIConstants.mismatchedCardColor)}
            }
        }
        else {
            ...
            for index in 0..<setCardsView.cardViews.count {
                let card = setCardsView.cardViews[index].card
                if game.cardsChosen.contains(card) { setCardsView.cardViews[index].setBorderColor(at: SetGameUIConstants.chosenCardColor)}
